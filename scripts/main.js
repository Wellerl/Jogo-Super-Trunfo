...
const shuffleDeck = () => {
  for (let i = deck.length - 1; i > 0; i--) {
    ...
    [deck[i], deck[j]] = [deck[j], deck[i]]; // Troca as posi es
  }
};

const drawCard = () => {
  if (deck.length < 2) return; // Ensure there are at least two cards
...
const attributes = ["strength", "defense", "magic"];
const computerAttributeName = attributes[Math.floor(Math.random() * attributes.length)];
const computerElement = document.querySelector(`.computer.${computerAttributeName}-value`);
const computerValue = Number(computerElement.textContent);
  ...
  const computerAttribute = computerElement
